...
void
qa_conv_decoder::test_simple_decode_3_4 ()
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<uint8_t> uni(0, 255);
  decoder::decoder_sptr dec = conv_decoder::make(conv_decoder::RATE_3_4, 2048);
  encoder::encoder_sptr  conv = conv_encoder::make(conv_encoder::RATE_3_4,
                                                   2048);
...
void
qa_conv_decoder::test_simple_decode_5_6 ()
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<uint8_t> uni(0, 255);
  decoder::decoder_sptr dec = conv_decoder::make(conv_decoder::RATE_5_6, 2048);
  encoder::encoder_sptr  conv = conv_encoder::make(conv_encoder::RATE_5_6,
                                                   2048);
...
void
qa_conv_decoder::test_simple_decode_7_8 ()
{
  std::random_device rd;
  std::mt19937 mt(rd());
  std::uniform_int_distribution<uint8_t> uni(0, 255);
  decoder::decoder_sptr dec = conv_decoder::make(conv_decoder::RATE_7_8, 2048);
  encoder::encoder_sptr  conv = conv_encoder::make(conv_encoder::RATE_7_8,
                                                   2048);
...
void
qa_conv_decoder::test_various_msg_len_3_4 ()
{
  for(uint32_t i = 4; i < 1024; i++) {
    test_decode(i, conv_decoder::RATE_3_4);
  }
}

void
qa_conv_decoder::test_various_msg_len_5_6 ()
{
  for(uint32_t i = 4; i < 1024; i++) {
    test_decode(i, conv_decoder::RATE_5_6);
  }
}

void
qa_conv_decoder::test_various_msg_len_7_8 ()
{
  for(uint32_t i = 4; i < 1024; i++) {
    test_decode(i, conv_decoder::RATE_7_8);
  }
}
